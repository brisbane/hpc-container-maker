...
class zypper(bb_base):
    ...
    epel: - TODO replace with suse things Boolean flag to specify whether to enable
    Extram repso The default is  False.
    ...
    powertools: TODO Boolean flag to specify whether to enable the
    PowerTools repository.  The default is False.  This parameter is
    only recognized if the CentOS version is 8.x.

    repositories: TODO A list of yum repositories to add.  The default is
    an empty list.

    scl: - TODO Boolean flag to specify whether to enable the Software
    Collections (SCL) repository.  The default is False.

    # Examples

    ```python
    zypper(ospackages=['make', 'wget'])
    ```
    ...
    def __init__(self, **kwargs):
        ...
        self.__download_directory = "/var/cache/zypp/packages"
        ...
        if hpccm.config.g_linux_distro != linux_distro.SUSE: # pragma: no cover
            logging.warning('Using zypper on a non-RHEL based Linux distribution')
